...
def save_manga(meta, path: str, cookie: str, saving_log=True, max_size=10000):
    save_img(meta, path, cookie, saving_log, max_size)


def save_ugoira(meta, path: str, cookie: str, save_gif: bool, saving_log=True):
...
def save_illust(pid: str, path: str, cookie: str, save_gif: bool, skip_download: bool, skip_meta: bool, saving_log=True):
    meta = spider.get_illust_meta(pid, cookie)
    if not skip_download:
        if meta["illustType"] == 0:
            save_img(meta, path, cookie, saving_log)
        elif meta["illustType"] == 1:
            save_manga(meta, path, cookie, saving_log)
        elif meta["illustType"] == 2:
            save_ugoira(meta, path, cookie, save_gif, saving_log)
        else:
